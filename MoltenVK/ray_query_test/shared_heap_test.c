// AS-lifecycle SMOKE TEST for MoltenVK (follow-up b). Exercises create/destroy/
// rebuild sequences that host.c does not, and confirms ray_query still returns
// hit=1 across them. Identical to host.c (1-triangle BLAS + 1-instance TLAS)
// plus:
//   1. a "decoy" AS whose backing buffer shares the SAME VkDeviceMemory (one
//      MTLHeap) as the real BLAS — drives the residency-refcount path (b1);
//   2. after the first build, the decoy is DESTROYED, then the TLAS is REBUILT
//      (the rebuild re-scans MVKDevice::getAccelerationStructureList()) — drives
//      the AS-list free-list path (b2).
//
// HONEST SCOPE: both b1 and b2 are DEFENSIVE hardening, not observable-failure
// bugs, so this test PASSES on the pre-fix code too — it is a smoke test, not a
// fails-before/passes-after reproduction. Why the bugs don't manifest in a
// minimal harness:
//   - b1 (residency-set drops a heap shared by a living sibling): masked because
//     every build AND dispatch ALSO covers AS heaps per-encode via
//     useHeapForResource / useResource (MVKCmdAccelerationStructure.mm,
//     MVKCommandEncoderState.mm), independent of the global residency set.
//   - b2 (released MTLAccelerationStructure left in _allAccStructs -> UAF on the
//     next TLAS build): masked because the MTLHeap retains its placement
//     sub-allocations, so the Metal object stays valid until the backing memory
//     is freed. The fix still removes a real latent UAF (free the memory while
//     the list holds the AS) + the documented unbounded growth.
// A true fails-before reproduction needs adversarial conditions (also freeing the
// backing VkDeviceMemory, slot-reuse pressure, or heavy concurrency).
//
// Build:  cc shared_heap_test.c -o shtest -I <Vulkan-Headers>/include -L/opt/homebrew/lib -lvulkan
// Run:    VK_DRIVER_FILES=<our MoltenVK_icd.json> ./shtest ray_query.comp.spv
...
// Create a buffer with NO memory bound; return it + its memory requirements,
// so the caller can sub-allocate several buffers into one shared VkDeviceMemory.
static VkBuffer mkBufferNoMem(VkDeviceSize sz, VkBufferUsageFlags usage, VkMemoryRequirements *req) {
    VkBufferCreateInfo bci = { VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO };
    bci.size = sz; bci.usage = usage; bci.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    VkBuffer b; VK_CHECK(vkCreateBuffer(dev, &bci, NULL, &b));
...
int main(int argc, char **argv) {
    ...
    const char *dexts[8]; uint32_t dn = 0;
    for (int i = 0; i < 4; i++) {
        int found = 0;
        for (uint32_t j = 0; j < aec; j++) if (!strcmp(want[i], aep[j].extensionName)) { found = 1; break; }
        if (found) dexts[dn++] = want[i];
        else fprintf(stderr, "device ext NOT present: %s\n", want[i]);
    }
    ...
    LOAD(pDestroyAS, DestroyAccelerationStructureKHR);
    #undef LOAD
    ...
    // BLAS storage + a DECOY AS storage buffer, both bound into ONE shared
    // VkDeviceMemory (= one MTLHeap). This is the crux of the shared-heap test.
    const VkBufferUsageFlags ASSTORE = VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR
                                     | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT;
    VkMemoryRequirements blasReq, decoyReq;
    VkBuffer blasBuf  = mkBufferNoMem(bsz.accelerationStructureSize, ASSTORE, &blasReq);
    VkBuffer decoyBuf = mkBufferNoMem(bsz.accelerationStructureSize, ASSTORE, &decoyReq);
    VkDeviceSize decoyOffset = (blasReq.size + decoyReq.alignment - 1) & ~(decoyReq.alignment - 1);
    VkDeviceMemory sharedMem;
    {
        ...
        VkMemoryAllocateInfo ai = { VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO };
        ai.pNext = &fi;
        ai.allocationSize = decoyOffset + decoyReq.size;
        ai.memoryTypeIndex = findMemPrivate(blasReq.memoryTypeBits & decoyReq.memoryTypeBits);
        VK_CHECK(vkAllocateMemory(dev, &ai, NULL, &sharedMem));
    }
    VK_CHECK(vkBindBufferMemory(dev, blasBuf,  sharedMem, 0));
    VK_CHECK(vkBindBufferMemory(dev, decoyBuf, sharedMem, decoyOffset));
    ...
    // Decoy AS on the shared heap. Creating it makes the shared heap resident a
    // SECOND time (refcount 2); it is never built — its only job is to be there,
    // then be destroyed below. No build => no scratch, no range.
    VkAccelerationStructureCreateInfoKHR decoyCI = { VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_CREATE_INFO_KHR };
    decoyCI.buffer = decoyBuf; decoyCI.size = bsz.accelerationStructureSize;
    decoyCI.type = VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR;
    VkAccelerationStructureKHR decoy; VK_CHECK(pCreateAS(dev, &decoyCI, NULL, &decoy));
    ...
    // THE TEST: destroy the decoy AS, then REBUILD the TLAS. The TLAS build scans
    // getAccelerationStructureList() and dereferences blas.heap on each entry —
    // so without the free-list fix the rebuild touches the decoy's released
    // MTLAccelerationStructure (use-after-free). The decoy also shared the BLAS's
    // heap, so its destruction exercises the residency refcount path too.
    printf("AS-lifecycle test: destroying decoy AS, then rebuilding TLAS...\n");
    pDestroyAS(dev, decoy, NULL);

    VkCommandBuffer cbRebuild = beginCmd();
    pCmdBuildAS(cbRebuild, 1, &tbi, &ptr);   // re-scans the AS list (the UAF site)
    endCmd(cbRebuild);
