// AS realistic-teardown test for MoltenVK (follow-up b2). The closest single-
// threaded approximation of renderer churn: destroy an AS, free its backing
// memory, then build another TLAS.
//
// host.c (1-triangle BLAS + 1-instance TLAS ray_query) plus a "decoy" BLAS on
// its OWN VkDeviceMemory:
//   1. build BLAS + TLAS;
//   2. destroy the decoy AS, then its buffer, then FREE its memory;
//   3. rebuild the TLAS — the build scans MVKDevice::getAccelerationStructure
//      List() and retains/dereferences every entry (MVKCmdAccelerationStructure.mm).
//
// HYPOTHESIS was that pre-fix (removeAccelerationStructure leaving the decoy's
// pointer in _allAccStructs, the // TODO) step 3 would touch a freed object ==
// use-after-free. EMPIRICAL RESULT: it does NOT fault, even on the pre-fix build
// run under NSZombieEnabled and under Guard Malloc (libgmalloc, which unmaps
// freed pages). So the latent UAF is NOT reachable through the public Vulkan API
// in this sequence on this MoltenVK/Metal/macOS — most likely the placement
// MTLAccelerationStructure / its heap outlives our release (deferred teardown or
// Metal-internal retention). The free-list fix is therefore confirmed DEFENSIVE
// (correct hygiene + the documented growth bound), not an observable-bug fix.
// This file stands as a clean realistic-teardown smoke test (returns hit=1).
//
// Build:  cc as_free_uaf_test.c -o shtest2 -I <Vulkan-Headers>/include -L/opt/homebrew/lib -lvulkan
// Run:    VK_DRIVER_FILES=<our MoltenVK_icd.json> ./shtest2 ray_query.comp.spv
//   (to retry the UAF hunt on a pre-fix build:
//    DYLD_INSERT_LIBRARIES=/usr/lib/libgmalloc.dylib ./shtest2)
...
int main(int argc, char **argv) {
    ...
    // Decoy BLAS on its OWN private memory (NOT shared with the real BLAS). It is
    // added to the device's AS list at creation, never built, and torn down below
    // — buffer + memory and all — so freeing its memory truly deallocates its
    // placement MTLAccelerationStructure.
    VkBuffer decoyBuf; VkDeviceMemory decoyMem;
    mkBuffer(bsz.accelerationStructureSize,
             VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR|VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
             1, 1, &decoyBuf, &decoyMem);
    ...
    // THE TEST: realistic teardown of the decoy, then a TLAS rebuild.
    // Destroy AS -> destroy buffer -> free memory. Freeing the memory releases the
    // decoy's MTLHeap (last owner of its placement MTLAccelerationStructure), so
    // that Metal object is now deallocated. Pre-fix, its pointer still sits in
    // _allAccStructs, so the rebuild's getAccelerationStructureList() scan touches
    // freed memory (use-after-free). The free-list fix nil'd the slot on destroy.
    printf("UAF test: destroy decoy AS, free its memory, then rebuild TLAS...\n");
    pDestroyAS(dev, decoy, NULL);
    vkDestroyBuffer(dev, decoyBuf, NULL);
    vkFreeMemory(dev, decoyMem, NULL);

    VkCommandBuffer cbRebuild = beginCmd();
    pCmdBuildAS(cbRebuild, 1, &tbi, &ptr);   // scans the AS list == the UAF site
    endCmd(cbRebuild);
